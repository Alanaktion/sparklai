...
def generate(self, prompt: str, negative_prompt: str | None = None) -> bytes:
if not self.enabled:
...
self.model = os.getenv('OPENAI_MODEL', 'llama-3-70b')
self.reply_chance = float(os.getenv('PUBLIC_REPLY_CHANCE', '0.2'))
self.post_interval = int(os.getenv('POST_INTERVAL_SECONDS', '1800'))
self.generate_images = os.getenv('ENABLE_IMAGE_GENERATION', 'false').lower() in {'1', 'true', 'yes'}
...
if not text:
return
self.masto.status_post(status=text[:500], **kwargs)
...
return None
try:
data = self.comfy.generate(prompt)
media = self.masto.media_post(data, mime_type='image/webp', description='Generated image')
return str(media['id'])
...
if status['account']['acct'] == self.profile.name:
return
clean = self._clean_html(status.get('content', ''))
