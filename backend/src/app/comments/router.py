...
async def generate_random_comment(db: DbDep):
    ...
    comment = await service.generate_comment_for_post(post, author)
...
async def respond_to_post(post_id: int, data: RespondRequest, db: DbDep):
    ...
    comment = await service.generate_comment_for_post(post, commenter)
    return CommentResponse.model_validate(comment)
...
async def translate_comment(post_id: int, comment_id: int, db: DbDep):
    body_en = await _service(db).translate_comment(post_id, comment_id)
